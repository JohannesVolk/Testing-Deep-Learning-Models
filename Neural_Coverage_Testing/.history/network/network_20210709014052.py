...
class Network(object):
    ...
    def evaluate(self, test_data,layers):
        ...
        max = cp.apply_along_axis(lambda x: cp.argmax(self.feedforward(x,layers)), 1 , data)
        print(cp.shape(cp.equal(max.reshape(len(max)), label)))

        print(cp.equal(cp.apply_along_axis(lambda x: cp.argmax(self.feedforward(x,layers)), 1 , data), label))

        return cp.count_nonzero(cp.equal(cp.apply_along_axis(lambda x: cp.argmax(self.feedforward(x,layers)), 1 , data), label))
