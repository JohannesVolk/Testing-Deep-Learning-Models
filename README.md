...
Even for a very basic model, it is clear that that classes activate the neurons in distinct patterns. Some classes have specific neurons whose activation stands out compared to neighboring neurons. Therefore, if this neurons are activated, but an unusual class is predicted, the user will be alerted about a lack of confidence in the predicted class probabilities.
